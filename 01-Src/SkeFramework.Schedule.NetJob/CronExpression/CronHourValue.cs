...
namespace SkeFramework.Schedule.NetJob.CronExpression
{
    public class CronHourValue
    {
        ...
        public static bool TryParse(string value, out CronHourValue crontabValue)
        {
            crontabValue = null;
            ...
            foreach (var item in array)
            {
                if (item == "*" || item == "*/1")
                {
                    crontabValue = new CronHourValue(null);
                    return true;
                }
            ...
            {
                crontabValue = new CronHourValue(list);
                return true;
            }
        }

        CronHourValue(List<CronNodeBase> list)
        {
            this.list = list;
        }
