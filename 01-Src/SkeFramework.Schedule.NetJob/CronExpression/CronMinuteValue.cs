...
namespace SkeFramework.Schedule.NetJob.CronExpression
{
    class CronMinuteValue
    {
        ...
        public static bool TryParse(string value, out CronMinuteValue crontabValue)
        {
            crontabValue = null;
            ...
            foreach (var item in array)
            {
                if (item == "*" || item == "*/1")
                {
                    crontabValue = new CronMinuteValue(null);
                    return true;
                }
            ...
            {
                crontabValue = new CronMinuteValue(list);
                return true;
            }
        }

        CronMinuteValue(List<CronNodeBase> list)
        {
            this.list = list;
        }
