...
namespace CodeBuilder.BLL
{
    ...
    /// UI层控制器实现【生成类】
    /// </summary>
    public sealed class CreateViewUpdate : CreateBase
    {
        public const int Type = 14;

        public const string Name = "{0}Update";

        public CreateViewUpdate()
            : base(CreateViewUpdate.Name, ".cshtml")
        {
        ...
        public override string CreateMethod( string TableName, string Namespace = "SkeFramework")
        {
            ...
            sb.AppendLine("@{");
            sb.AppendLine("    ViewBag.Title = \"修改\";");
            ...
            sb.AppendLine(string.Format("    <h1>{0}修改</h1>", DealTableName));
            ...
            sb.AppendLine(string.Format("                updatePostUrl: \"@Url.Action(\"{0}Update\", \"{0}\")\",", DealTableName));
            sb.AppendLine(string.Format("                getItemUrl: \"@Url.Action(\"Get{0}Info\", \"{0}\")\",", DealTableName));
            sb.AppendLine("                dataItem: {},");
            sb.AppendLine("                id:\"@Request.QueryString[\"id\"].ToString()\"");
            sb.AppendLine("            },");
            ...
            sb.AppendLine("                    _self.dataItem = {};");
            sb.AppendLine("                    axios.get(this.getItemUrl,{ params: {id: this.id } } )");
            sb.AppendLine("                     .then(function (response) {");
            sb.AppendLine("                         if (response.status == 200) {");
            sb.AppendLine("                             _self.dataItem = response.data;");
            sb.AppendLine("                         }");
            sb.AppendLine("                     })");
            ...
            sb.AppendLine("                    var _self = this;");
            sb.AppendLine("                    axios.post(this.updatePostUrl, this.dataItem)");
            sb.AppendLine("                     .then(function (response) {");
            sb.AppendLine("                         if (response.status == 200) {");
            sb.AppendLine("                             alert(response.data.ResultMsg);");
            sb.AppendLine("                             if (response.data.ResultCode == 200) {");
